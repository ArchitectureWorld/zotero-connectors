...
This preserves the existing toolbar-button path and its official fallback behavior.
...
The native host is allowlisted to the packaged extension ID. Pipe clients authenticate with a randomly generated per-user key stored under `%LOCALAPPDATA%\ZoteroScriptTrigger`.
...
The consumer must require the returned URL to equal the requested URL exactly.
...
- Windows EXE/package assembly.

## Manual evidence

The user successfully installed and exercised the earlier packaged build in Windows Chrome with Zotero, including foreground/background operation. Protocol-v2 exact-URL targeting is a later extension and requires one final V3 package check on the user's machine before the PR should leave draft state.
...
- Chrome-discarded tabs are not force-reloaded by protocol v2; callers should keep target CNKI detail tabs loaded.
